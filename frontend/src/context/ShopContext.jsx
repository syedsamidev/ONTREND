...
const ShopContextProvider = (props) =>{
    ...
    const getCartCount = () => {
        ...
        for (const itemId in cartItems) {
            for (const size in cartItems[itemId]) {
                // Ensure size count exists and is greater than 0
    ...
    const updateQuantity1 = async ({itemId,size,quantity}) => {
        let cartData = structuredClone(cartItems);
    ...
    const updateQuantity = async ({itemId, size, quantity}) => {
        ...
        if (quantity === 0) {
            // Delete the size from the frontend cart state
            delete cartData[itemId][size];
            if (Object.keys(cartData[itemId]).length === 0) {
                delete cartData[itemId]; // Delete the item if no sizes are left
            }
        } else {
        ...
        if (token) {
            ...
            // Guest user: Store updated cart in localStorage
            localStorage.setItem('guestCart', JSON.stringify(cartData));
        }
    };
    ...
    const getProductsData = async () => {
        try {
            ...
            if (response.data.success) {
                setProducts(response.data.products); // Use the correct field
            } else {
